        ...
        Pass
        {
            ...
            fixed4 frag (v2f i) : SV_Target
            {
                ...
                if (CombatLink_IsActive())
                {
                    ...
                    {
                        col = lerp(col,  half4(1, 0, 0, col.a + _AlphaAdd), (i.gradient.x < combatLinkDataMain.r) * mask);
                    }
                    ...
                    if (distance(i.mask, float2(0.01f, 1)) < 0.005f)
                    {
                        ...
                        {
                            col = lerp(col, half4(0, 1, 0, col.a + _AlphaAdd), (i.gradient.x < combatLinkDataMain.g) * mask);
                        }
                        else {
                            col.a = 0;
                        }
                    ...
                    {
                        col = lerp(col, half4(0.75f, 0.75f, 1, col.a + _AlphaAdd), (i.gradient.x < combatLinkDataMain.b) * mask);
                    }
                    ...
                    if (distance(i.mask, float2(0.03f, 1)) < 0.005f)
                    {
                        ...
                        if (temperature > 0)
                        {
                            col = lerp(col, half4(lerp(half3(0.5,1,0.5), half3(0.5, 0, 0), clamp((temperature*2) - 0.1,0,1)), col.a + _AlphaAdd), (i.gradient.x < temperature) * mask);

                        }
                        else {
                            col = lerp(col, half4(lerp(half3(0.5, 1, 0.5), half3(0.5, 0.5, 1), clamp((-2 * temperature)-0.05, 0,1)), col.a + _AlphaAdd), (i.gradient.x < -1 * temperature) * mask);
                        }
                    }
